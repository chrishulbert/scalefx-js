...
export function scale3x(image: Image): Image {
...
const THRESHOLD: number = 0.5; // Min 0.01; max: 1; step: 0.01
const IS_FILTER_AA_ENABLED: boolean = true;
const FILTER_CORNERS: boolean = true; // SFX_SCN in the shader.

type Pixel = {
    p: number, // 0xRRGGBBAA pixel.
    dx: number, // Colour distance.
    dy: number,
    dz: number,
    dw: number,
    sx: number, // Corner strength.
    sy: number,
    sz: number,
    sw: number,
    r: VecB4, // I think this means 'resolution'.
    h: VecB4, // Horizontal edges.
    v: VecB4, // Vertical edges.
    o: VecB4, // Orientation.
    c: Vec4, // Corners.
    m: Vec4, // Mids.
};
...
function p(p: number): Pixel {
    const EMPTY_V4: Vec4 = { x: 0, y: 0, z: 0, w: 0 };
    const EMPTY_VB4: VecB4 = { x: false, y: false, z: false, w: false };
    return {
        p,
        dx: 0,
        dy: 0,
...
function add_transparent_border(image: Image): IntermediateImage {
    const new_width = image.width + 2;
    const new_height = image.height + 2;
    const out: Pixel[] = [];
    for (let x=0; x<new_width; x++) { out.push(p(0)); } // Top row.
    ...
    for (let x=0; x<new_width; x++) { out.push(p(0)); } // Bottom row.
    return {
        w: new_width,
        h: new_height,
        p: out,
...
function remove_transparent_border(image: Image): Image {
...
function calculate_distances(m: IntermediateImage): IntermediateImage {
    ...
    function d(x: number, y: number) {
        ...
        const yb = (y >> 8) & 0xff;
        const ya = y & 0xff;
        if (xa < 0x80 && ya < 0x80) { return 0. } // Transparent vs transparent counts as the same.
        if (xa < 0x80 || ya < 0x80) { return 1. } // Colour -> transparent counts as different.
        const rm = (xr + yr) / 2; // Red mean.
        const r = Math.abs(xr - yr);
        const g = Math.abs(xg - yg);
        const b = Math.abs(xb - yb);
        if (r == 0 && g == 0 && b == 0) { return 0. } // Shortcut when all same.
...
function calculate_corner_strengths(m: IntermediateImage): IntermediateImage {
    function s(d: number, ax: number, ay: number, bx: number, by: number): number {
        let f = ax - ay; // Diff.
...
function resolve_corner_configurations(m: IntermediateImage): IntermediateImage {
    // Corner dominance at junction:
    function cd(x: Vec3, y: Vec3, z: Vec3, w: Vec3): Vec4 {
    ...
    function cl(c: Vec2, a: Vec2, b: Vec2): number {
    ...
    for (let y=0; y<m.h; y++) {
        for (let x=0; x<m.w; x++) {
            ...
            e.o = vb4v(v4ge(
                v4a(ho, v4(d.dw, e.dw, e.dw, d.dw)),
                v4a(v,  v4(e.dy, e.dy, h.dy, h.dy))));
        }
    }
    return m;
}
...
function determine_edge_levels(m: IntermediateImage): IntermediateImage {
    let t = EMPTY_PIXEL;
    for (let y=0; y<m.h; y++) {
        for (let x=0; x<m.w; x++) {
            // Get neighbours:
            ...
            // Extract data:            
            const ec = e.r; const eh = e.h; const ev = e.v; const eo = e.o;
            const dc = d.r; const dh = d.h; const dr = d.o; const d0c = d0.r; const d0h = d0.h; const d1h = d1.h;
            const fc = f.r; const fh = f.h; const fo = f.o; const f0c = f0.r; const f0h = f0.h; const f1h = f1.h;
            const bc = b.r; const bv = b.v; const bo = b.o; const b0c = b0.r; const b0v = b0.v; const b1v = b1.v;
            const hc = h.r; const hv = h.v; const ho = h.o; const h0c = h0.r; const h0v = h0.v; const h1v = h1.v;
...
function scale_subpixels(m: IntermediateImage): Image {
    const pixels: number[] = [];
    const row0: number[] = [];
    const row1: number[] = [];
    const row2: number[] = [];
        ...
        for (let x=0; x<m.w; x++) {
            ...
            for (let spy=0; spy<3; spy++) { // Loop the subpixels.
                for (let spx=0; spx<3; spx++) {
                    ...
                    let oy = 0;
                    if (sp==0) { ox=0; oy=0 }
                    else if (sp==1) { ox=-1; oy=0 }
                    else if (sp==2) { ox=-2; oy=0 }
                    else if (sp==3) { ox=1; oy=0 }
                    else if (sp==4) { ox=2; oy=0 }
                    else if (sp==5) { ox=0; oy=-1 }
                    else if (sp==6) { ox=0; oy=-2 }
                    else if (sp==7) { ox=0; oy=1 }
                    else if (sp==8) { ox=0; oy=2 }
                    ...
                    if (spy==0) { row0.push(colour) }
                    else if (spy==1) { row1.push(colour) }
                    else { row2.push(colour) }
                }
            }
        }
        pixels.push(...row0);
...
type VecB4 = { x: boolean, y: boolean, z: boolean, w: boolean };
type VecB2 = { x: boolean, y: boolean };
...
function vb2(x: boolean, y: boolean): VecB2 {
  return {x, y};
};
function vb4(x: boolean, y: boolean, z: boolean, w:boolean): VecB4 {
  return {x, y, z, w};
};
function vb4v(v: Vec4): VecB4 { // VecB4 from a Vec4.
    return {x: v.x > 0.5, y: v.y > 0.5, z: v.z > 0.5, w: v.w > 0.5};
}
// Multiply by a number.
function v4m(v: Vec4, a: number): Vec4 {
    return {x: v.x * a, y: v.y * a, z: v.z * a, w: v.w * a};
}
// Multiply by another vec.
function v4mv(a: Vec4, b: Vec4): Vec4 {
    return {x: a.x * b.x, y: a.y * b.y, z: a.z * b.z, w: a.w * b.w};
}
function v4a(a: Vec4, b: Vec4): Vec4 { // Add.
    return {x: a.x + b.x, y: a.y + b.y, z: a.z + b.z, w: a.w + b.w};
}
function v4s(a: Vec4, b: Vec4): Vec4 { // Subtract.
    return {x: a.x - b.x, y: a.y - b.y, z: a.z - b.z, w: a.w - b.w};
}
function wxyz(v: Vec4): Vec4 {
    return v4(v.w, v.x, v.y, v.z);
}
function yzwx(v: Vec4): Vec4 {
    return v4(v.y, v.z, v.w, v.x);
}
function zwxy(v: Vec4): Vec4 {
    return v4(v.z, v.w, v.x, v.y);
}
function v4ge(a: Vec4, b: Vec4): Vec4 {
    return v4s(v4(1,1,1,1), v4step(a, b));
}
function v4le(a: Vec4, b: Vec4): Vec4 {
    return v4s(v4(1,1,1,1), v4step(b, a));
}
function v4leq(a: Vec4, b: Vec4): Vec4 {
    return v4step(a, b);
}
function v4step(e: Vec4, x: Vec4): Vec4 {
    return {
        x: x.x < e.x ? 0 : 1,
        y: x.y < e.y ? 0 : 1,
        z: x.z < e.z ? 0 : 1,
...
function nstep(e: number, x: number): number {
    return x < e ? 0 : 1
}
function nge(x: number, y: number): number {
    return 1 - nstep(x, y)
}
function v4n(v: Vec4): Vec4 { // Not.
